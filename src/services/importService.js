...
const buildSqlScript = (records) => {
  const createTableSql = `CREATE TABLE IF NOT EXISTS mock_data (
  ...
  const insertStatements = records.map((record) => {
    const jsonData = JSON.stringify(record).replace(/'/g, "''");
    return `INSERT INTO mock_data (data) VALUES ('${jsonData}');`;
  });

  return [createTableSql, ...insertStatements].join('\n');
};

const importFile = async (file, mode = 'import') => {
  ...
  if (mode === 'sql') {
    return { sql: buildSqlScript(cleanedRecords) };
  }
